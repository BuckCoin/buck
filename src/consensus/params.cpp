...
namespace Consensus {
    ...
    int Params::Halving(int nHeight) const {
        ...
        if (NetworkUpgradeActive(nHeight, Consensus::UPGRADE_BLOSSOM)) {
            ...
            int64_t scaledHalvings = ((blossomActivationHeight) * Consensus::BLOSSOM_POW_TARGET_SPACING_RATIO)
    ...
    int Params::HalvingHeight(int nHeight, int halvingIndex) const {
       ...
       // zip208
        ...
        if (NetworkUpgradeActive(nHeight, Consensus::UPGRADE_BLOSSOM)) {
            ...
            return
                ((1000000) * halvingIndex)
                - (BLOSSOM_POW_TARGET_SPACING_RATIO * (blossomActivationHeight))
                + blossomActivationHeight + (1000000);
        } else {
            return (nPreBlossomSubsidyHalvingInterval * halvingIndex);
        }
    }
