...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:5739/\n";
}
...
std::string experimentalDisabledHelpMsg(const std::string& rpc, const std::vector<string>& enableArgs)
{
    ...
    return "\nWARNING: " + rpc + " is disabled.\n" +
        ...
        + cmd + "\n\n" +
        "Alternatively add these two lines to the buck.conf file:\n\n"
        + config;
}
