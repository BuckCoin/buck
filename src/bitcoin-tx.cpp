...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Buck zcash-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  zcash-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded buck transaction") + "\n" +
              "  zcash-tx [options] -create [commands]   " + _("Create hex-encoded buck transaction") + "\n" +
              "\n";
