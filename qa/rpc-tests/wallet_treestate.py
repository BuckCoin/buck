...
class WalletTreeStateTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        recipients = []
        recipients.append({"address":myzaddr, "amount":Decimal('1000.0') - Decimal('0.0001')})
        ...
        assert_equal(Decimal(resp), Decimal('999.9999') * 3 )
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress('sprout'), "amount":Decimal('1000.0') - Decimal('0.0001')})
        myopid = self.nodes[0].z_sendmany(mytaddr, recipients)
        wait_and_assert_operationid_status(self.nodes[0], myopid)
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress('sprout'), "amount":Decimal('2080')})
